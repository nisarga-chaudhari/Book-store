...
function adminModel()
{
    ...
    this.addcategory=(collection_name,booktype)=>{
        return new Promise((resolve,reject)=>{
            db.collection(collection_name).find().toArray((err,result)=>{
                ...
                else
                {
                    ...
                    booktype._id=_id+1
                    db.collection(collection_name).insertOne(booktype,(err,result)=>{
                        err ? reject(err) : resolve(result);        
                    })
                }            
            })
    ...
    this.fetchbooktype=(collection_name)=>{
        return new Promise((resolve,reject)=>{
            db.collection(collection_name).find().toArray((err,result)=>{
                err ? reject(err) : resolve(result)  
            })
    ...
    this.deletebook=(collection_name,book_id)=>{
        return new Promise((resolve,reject)=>{
            db.collection(collection_name).deleteOne(book_id,(err,result)=>{
                err ? reject(err) : resolve(result)  
            })
        })
    }
